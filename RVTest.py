...
pulse_delay = 2e-5
smu.source_voltage = readV
smu.write(f"SOUR{smu.ch}:PULS:DEL {pulse_delay}") # 20 µs pulse delay
smu.write(f"SOUR:puls:widt {pulse_width}")
...
smu.write(f"TRIG{smu.ch}:ACQ:DEL {pulse_delay+0.5*pulse_width}")
...
smu.write(f"TRAC{smu.ch}:TST:FORM ABS")
