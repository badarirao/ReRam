...
class app_Fatigue(Ui_Fatigue):
    ...
    def __init__(self, parent=None, smu = None, k2700 = None, afg1022 = None, sName="Sample_Fatigue", connection=1):
        ...
        self.set_pulseWidth.valueChanged.connect(self.update_resetPulse)
        self.reset_pulseWidth.valueChanged.connect(self.update_setPulse)
    ...
    def update_resetPulse(self):
        if self.source.currentText() == "Keysight B2902B":
            self.reset_pulseWidth.setValue(self.set_pulseWidth.value())
        self.update_total_time()

    def update_setPulse(self):
        if self.source.currentText() == "Keysight B2902B":
            self.set_pulseWidth.setValue(self.reset_pulseWidth.value())
        self.update_total_time()
    ...
    def initialize_thread(self):
        self.thread = QThread()
        self.worker = Worker(self.smu, self.k2700, self.afg1022, self.sourceID,
                             self.connection, self.params, self.fullfilename)
...
class Worker(QObject):
    ...
    stopcall = pyqtSignal()
    def __init__(self, smu = None, k2700 = None, afg = None, sourceID = "B2902B",
                 connection = 1, params = [], fullfilename = "sample.dat"):
    ...
    def pulseMeasure_AFG(self):
        ...
        """
        while self.i < self.number_of_points:
            ...
            self.HRS.append(HRS)
            self.data.emit([self.points[self.i], HRS, LRS])
            self.i = self.i + 1
    ...
    def start_Fatigue(self):
        ...
        self.totalTime = self.points[-1] * (self.setTimestep + self.resetTimestep)
        self.timeTaken.setText("Total experiment time: {}".format(str(timedelta(seconds=int(self.totalTime)))))
        if self.sourceID == 'AFG1022':
            connect_sample_with_AFG(self.k2700, self.connection)
            self.pulseMeasure_AFG()
        elif self.sourceID == "B2902B":
            self.pulseMeasure_B2902B()
        self.saveData()
        self.finished.emit()
    ...
    def read_LRS_HRS_states(self):
        ...
        sleep(0.2)  # wait for 20msec to ensure switching is complete
        ...
        if not filePresent:
            with open(self.fullfilename, "w", newline='') as f:
                ...
                if self.smu.ID == "B2902B":
                    f.write("##Resistance read using Keysight B2902B SMU.\n")
                ...
                f.write(f"##Set voltage = {self.params['Vset']}, Reset Voltage = {self.params['Vreset']}.\n")
                f.write(f"##Set pulse width = {self.setTimestep}s, Reset pulse width = {self.resetTimestep}s\n")
                f.write(f"##Read voltage = {self.params['Rvoltage']}V, averaged over {self.params['Average']} readings\n")
                f.write(f"##Limiting current = {self.iLimit.value()}mA\n")
